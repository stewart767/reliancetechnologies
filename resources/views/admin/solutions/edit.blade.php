...
<div class="space-y-8 max-w-3xl">
    <div class="flex items-center justify-between flex-wrap gap-4">
        <div class="space-y-2">
            <h1 class="font-title font-extrabold text-2xl md:text-3xl text-white">Edit Solution</h1>
            <p class="text-xs sm:text-sm text-slate-500">Edit parameters for: <strong class="text-slate-350">{{ $solution->title }}</strong></p>
        </div>
    ...
    <div class="bg-slate-900 border border-slate-800 rounded-xl p-6 md:p-8 shadow-lg">
        <form action="{{ route('admin.solutions.update', $solution->id) }}" method="POST" enctype="multipart/form-data" class="space-y-6">
            @csrf
            @method('PUT')
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                ...
                <div class="space-y-2">
                    ...
                    <input type="text" name="slug" id="slug" value="{{ old('slug', $solution->slug) }}" required
                           class="w-full bg-slate-950 border border-slate-800 rounded-lg px-4 py-2.5 text-sm focus:outline-none focus:border-blue-500 transition-colors text-white">
                </div>
            </div>
            ...
            <div class="space-y-2">
                ...
                <input type="text" name="subtitle" id="subtitle" value="{{ old('subtitle', $solution->subtitle) }}" required
                       class="w-full bg-slate-950 border border-slate-800 rounded-lg px-4 py-2.5 text-sm focus:outline-none focus:border-blue-500 transition-colors text-white">
            </div>
            ...
            <div class="space-y-2">
                ...
                <textarea name="short_description" id="short_description" rows="3" required
                          class="w-full bg-slate-950 border border-slate-800 rounded-lg px-4 py-2.5 text-sm focus:outline-none focus:border-blue-500 transition-colors text-white resize-y">{{ old('short_description', $solution->short_description) }}</textarea>
            </div>
            ...
            <div class="space-y-2">
                ...
                <textarea name="description" id="description" rows="6" required
                          class="w-full bg-slate-950 border border-slate-800 rounded-lg px-4 py-2.5 text-sm focus:outline-none focus:border-blue-500 transition-colors text-white resize-y">{{ old('description', $solution->description) }}</textarea>
            </div>

            <!-- Dynamic Capabilities Array with Alpine.js -->
            <div class="space-y-4" x-data="{ capabilities: {{ json_encode(old('capabilities', $solution->capabilities ?: [''])) }} }">
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6 pt-4">
                <div class="space-y-2">
                    ...
                    <input type="number" name="sort_order" id="sort_order" value="{{ old('sort_order', $solution->sort_order) }}" required
                           class="w-full bg-slate-950 border border-slate-800 rounded-lg px-4 py-2.5 text-sm focus:outline-none focus:border-blue-500 transition-colors text-white">
                </div>
            ...
            <div class="flex items-center gap-6 pt-4 border-t border-slate-800">
                <div class="flex items-center">
                    <input type="checkbox" name="is_featured" id="is_featured" value="1" {{ old('is_featured', $solution->is_featured) ? 'checked' : '' }}
