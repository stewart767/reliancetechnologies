...
<div class="space-y-8 max-w-3xl">
    <div class="flex items-center justify-between flex-wrap gap-4">
        <div class="space-y-2">
            <h1 class="font-title font-extrabold text-2xl md:text-3xl text-white">Edit Product</h1>
            <p class="text-xs sm:text-sm text-slate-500">Update product specifications, files, assets, and descriptions.</p>
        </div>
    ...
    <div class="bg-slate-900 border border-slate-800 rounded-xl p-6 md:p-8 shadow-lg">
        <form action="{{ route('admin.products.update', $product->id) }}" method="POST" enctype="multipart/form-data" class="space-y-6"
              x-data="{ type: '{{ old('type', $product->type) }}', features: {{ json_encode(old('features', $product->features ?: [''])) }}, specs: {{ json_encode(old('specs', $product->specs ?: [''])) }} }">
            @csrf
            @method('PUT')
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                ...
                <div class="space-y-2">
                    ...
                    <input type="text" name="title" id="title" value="{{ old('title', $product->title) }}" required placeholder="e.g. Smart Sale POS"
                           class="w-full bg-slate-950 border border-slate-800 rounded-lg px-4 py-2.5 text-sm focus:outline-none focus:border-blue-500 transition-colors text-white">
                </div>
            </div>
            ...
            <div class="space-y-2">
                ...
                <textarea name="description" id="description" rows="5" required placeholder="Describe product details, benefits, and target market..."
                          class="w-full bg-slate-950 border border-slate-800 rounded-lg px-4 py-2.5 text-sm focus:outline-none focus:border-blue-500 transition-colors text-white resize-y">{{ old('description', $product->description) }}</textarea>
            </div>
            ...
            <div class="space-y-2" x-show="type === 'software'">
                ...
                <input type="text" name="website_url" id="website_url" value="{{ old('website_url', $product->website_url) }}" placeholder="e.g. https://www.smartsale.co.tz"
                       class="w-full bg-slate-950 border border-slate-800 rounded-lg px-4 py-2.5 text-sm focus:outline-none focus:border-blue-500 transition-colors text-white">
            </div>
            ...
            <div class="space-y-2" x-show="type === 'hardware'">
                ...
                <input type="text" name="price" id="price" value="{{ old('price', $product->price) }}" placeholder="e.g. TZS 2,400,000"
                       class="w-full bg-slate-950 border border-slate-800 rounded-lg px-4 py-2.5 text-sm focus:outline-none focus:border-blue-500 transition-colors text-white">
            </div>
            ...
            <div class="grid grid-cols-1 md:grid-cols-3 gap-6">
                <div class="space-y-2 md:col-span-2">
                    ...
                    <input type="text" name="icon" id="icon" value="{{ old('icon', $product->icon) }}" placeholder='<svg class="w-6 h-6 text-blue-500" fill="none" ...></svg>'
                           class="w-full bg-slate-950 border border-slate-800 rounded-lg px-4 py-2.5 text-sm focus:outline-none focus:border-blue-500 transition-colors text-white">
                </div>
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                <div class="space-y-2">
                    <label for="image" class="block text-xs font-bold text-slate-400 uppercase tracking-wider">Change Product Photo</label>
                    <input type="file" name="image" id="image" accept="image/*"
                           class="w-full text-xs text-slate-400 file:mr-4 file:py-2 file:px-4 file:rounded-md file:border-0 file:text-xs file:font-semibold file:bg-blue-600/10 file:text-blue-500 hover:file:bg-blue-650/20 mb-3">
                    
                    @if($product->image)
                        <div class="flex items-center gap-3 bg-slate-950 p-3 rounded-lg border border-slate-800 w-fit">
                            <img src="{{ Str::startsWith($product->image, 'images/') ? asset($product->image) : asset('storage/' . $product->image) }}" 
                                 alt="{{ $product->title }}" 
                                 class="w-16 h-16 object-cover rounded-lg border border-slate-800">
