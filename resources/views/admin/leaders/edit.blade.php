...
<div class="space-y-8 max-w-3xl">
    <div class="flex items-center justify-between flex-wrap gap-4">
        <div class="space-y-2">
            <h1 class="font-title font-extrabold text-2xl md:text-3xl text-white">Edit Board Member</h1>
            <p class="text-xs sm:text-sm text-slate-500">Edit settings for: <strong class="text-slate-350">{{ $leader->name }}</strong></p>
        </div>
    ...
    <div class="bg-slate-900 border border-slate-800 rounded-xl p-6 md:p-8 shadow-lg">
        <form action="{{ route('admin.leaders.update', $leader->id) }}" method="POST" enctype="multipart/form-data" class="space-y-6">
            @csrf
            @method('PUT')
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                <div class="space-y-2">
                    ...
                    <input type="text" name="name" id="name" value="{{ old('name', $leader->name) }}" required
                ...
                <div class="space-y-2">
                    ...
                    <input type="text" name="role" id="role" value="{{ old('role', $leader->role) }}" required
                           class="w-full bg-slate-950 border border-slate-800 rounded-lg px-4 py-2.5 text-sm focus:outline-none focus:border-blue-500 transition-colors text-white">
                </div>
            </div>
            ...
            <div class="space-y-2">
                ...
                <textarea name="bio" id="bio" rows="5" required
                          class="w-full bg-slate-950 border border-slate-800 rounded-lg px-4 py-2.5 text-sm focus:outline-none focus:border-blue-500 transition-colors text-white resize-y">{{ old('bio', $leader->bio) }}</textarea>
            </div>
            ...
            <div class="grid grid-cols-1 md:grid-cols-3 gap-6">
                <div class="space-y-2">
                    ...
                    <input type="number" name="experience_years" id="experience_years" value="{{ old('experience_years', $leader->experience_years) }}" required
                           class="w-full bg-slate-950 border border-slate-800 rounded-lg px-4 py-2.5 text-sm focus:outline-none focus:border-blue-500 transition-colors text-white">
                </div>

                <div class="space-y-2">
                    <label for="location" class="block text-xs font-bold text-slate-400 uppercase tracking-wider">Location / Label *</label>
                    <input type="text" name="location" id="location" value="{{ old('location', $leader->location) }}" required
                           class="w-full bg-slate-950 border border-slate-800 rounded-lg px-4 py-2.5 text-sm focus:outline-none focus:border-blue-500 transition-colors text-white">
                </div>

                <div class="space-y-2">
                    <label for="sort_order" class="block text-xs font-bold text-slate-400 uppercase tracking-wider">Sort Order Index</label>
                    <input type="number" name="sort_order" id="sort_order" value="{{ old('sort_order', $leader->sort_order) }}" required
                           class="w-full bg-slate-950 border border-slate-800 rounded-lg px-4 py-2.5 text-sm focus:outline-none focus:border-blue-500 transition-colors text-white">
                </div>
            </div>
            ...
            <div class="space-y-2">
                ...
                @if($leader->avatar)
                    <div class="flex items-center gap-4 mb-2">
                        <img src="{{ asset('storage/' . $leader->avatar) }}" alt="{{ $leader->name }}" class="h-14 w-14 rounded-full object-cover border border-slate-800">
            ...
            <div class="flex items-center pt-2">
                <input type="checkbox" name="is_active" id="is_active" value="1" {{ old('is_active', $leader->is_active) ? 'checked' : '' }}
