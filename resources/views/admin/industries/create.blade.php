...
<div class="space-y-8 max-w-3xl">
    <div class="flex items-center justify-between flex-wrap gap-4">
        <div class="space-y-2">
            <h1 class="font-title font-extrabold text-2xl md:text-3xl text-white">Create Industry</h1>
            <p class="text-xs sm:text-sm text-slate-500">Draft a new industry sector mapping segment.</p>
        </div>
        ...
        <form action="{{ route('admin.industries.store') }}" method="POST" class="space-y-6">
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                <div class="space-y-2">
                    ...
                    <input type="text" name="name" id="name" value="{{ old('name') }}" required placeholder="e.g. Mining Operations"
                ...
                <div class="space-y-2">
                    ...
                    <input type="text" name="slug" id="slug" value="{{ old('slug') }}" placeholder="e.g. mining-operations"
                           class="w-full bg-slate-950 border border-slate-800 rounded-lg px-4 py-2.5 text-sm focus:outline-none focus:border-blue-500 transition-colors text-white">
                </div>
            </div>
            ...
            <div class="space-y-2">
                ...
                <textarea name="challenge" id="challenge" rows="3" required placeholder="Frictions currently faced by this sector..."
            ...
            <div class="space-y-2">
                ...
                <textarea name="opportunity" id="opportunity" rows="3" required placeholder="Friction removal opportunities..."
                          class="w-full bg-slate-950 border border-slate-800 rounded-lg px-4 py-2.5 text-sm focus:outline-none focus:border-blue-500 transition-colors text-white resize-y">{{ old('opportunity') }}</textarea>
            </div>
            ...
            <div class="space-y-2">
                ...
                <textarea name="solution_desc" id="solution_desc" rows="4" required placeholder="How Reliance integrates specific software/hardware to solve this..."
                          class="w-full bg-slate-950 border border-slate-800 rounded-lg px-4 py-2.5 text-sm focus:outline-none focus:border-blue-500 transition-colors text-white resize-y">{{ old('solution_desc') }}</textarea>
            </div>
