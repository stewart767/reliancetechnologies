...
<div class="space-y-8 max-w-3xl">
    <div class="flex items-center justify-between flex-wrap gap-4">
        <div class="space-y-2">
            <h1 class="font-title font-extrabold text-2xl md:text-3xl text-white">Edit Homepage Slide</h1>
            <p class="text-xs sm:text-sm text-slate-500">Modify the configuration, images, and action links of this slideshow hero slide.</p>
        </div>
    ...
    <div class="bg-slate-900 border border-slate-800 rounded-xl p-6 md:p-8 shadow-lg">
        <form action="{{ route('admin.sliders.update', $slider->id) }}" method="POST" enctype="multipart/form-data" class="space-y-6">
            ...
            <div class="space-y-2">
                ...
                <input type="text" name="title" id="title" value="{{ old('title', $slider->title) }}" placeholder="e.g. Technology That Moves Your Business Forward."
            ...
            <div class="space-y-2">
                ...
                <textarea name="description" id="description" rows="3" placeholder="Provide subtitle, sub-heading, or description details displayed below title..."
                          class="w-full bg-slate-950 border border-slate-800 rounded-lg px-4 py-2.5 text-sm focus:outline-none focus:border-blue-500 transition-colors text-white resize-y">{{ old('description', $slider->description) }}</textarea>
            </div>

            <!-- CTA Primary Group -->
            <div class="bg-slate-955/50 p-4 border border-slate-800/60 rounded-lg space-y-4">
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                    <div class="space-y-2">
                        ...
                        <input type="text" name="primary_cta_text" id="primary_cta_text" value="{{ old('primary_cta_text', $slider->primary_cta_text) }}" placeholder="e.g. Talk to an Expert"
                    ...
                    <div class="space-y-2">
                        ...
                        <input type="text" name="primary_cta_url" id="primary_cta_url" value="{{ old('primary_cta_url', $slider->primary_cta_url) }}" placeholder="e.g. /contact"
                               class="w-full bg-slate-950 border border-slate-800 rounded-lg px-4 py-2 text-sm focus:outline-none focus:border-blue-500 transition-colors text-white">
                    </div>
                </div>
            </div>
            ...
            <div class="bg-slate-955/30 p-4 border border-slate-850 rounded-lg space-y-4">
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                    <div class="space-y-2">
                        ...
                        <input type="text" name="secondary_cta_text" id="secondary_cta_text" value="{{ old('secondary_cta_text', $slider->secondary_cta_text) }}" placeholder="e.g. Explore Solutions"
                               class="w-full bg-slate-950 border border-slate-800 rounded-lg px-4 py-2 text-sm focus:outline-none focus:border-blue-500 transition-colors text-white">
                    </div>
                    <div class="space-y-2">
                        ...
                        <input type="text" name="secondary_cta_url" id="secondary_cta_url" value="{{ old('secondary_cta_url', $slider->secondary_cta_url) }}" placeholder="e.g. #solutions"
                               class="w-full bg-slate-950 border border-slate-800 rounded-lg px-4 py-2 text-sm focus:outline-none focus:border-blue-500 transition-colors text-white">
                    </div>
                </div>
            </div>

            <!-- Current Asset Preview & Image Upload -->
            <div class="grid grid-cols-1 md:grid-cols-3 gap-6">
                <div class="space-y-2">
                    ...
                    <input type="number" name="sort_order" id="sort_order" value="{{ old('sort_order', $slider->sort_order) }}" required
                           class="w-full bg-slate-950 border border-slate-800 rounded-lg px-4 py-2.5 text-sm focus:outline-none focus:border-blue-500 transition-colors text-white">
                </div>

                <div class="md:col-span-2 space-y-2">
                    <label for="background_image" class="block text-xs font-bold text-slate-400 uppercase tracking-wider">Background Image File (Leave blank to keep current)</label>
                    
                    @if($slider->background_image)
                        <div class="flex items-center gap-4 mb-3 p-2 bg-slate-950 rounded border border-slate-850">
                            <img src="{{ Str::startsWith($slider->background_image, 'images/') ? asset($slider->background_image) : asset('storage/' . $slider->background_image) }}" 
                                 alt="Current slide" 
                                 class="h-14 w-24 object-cover rounded border border-slate-800">
                            <div class="text-[10px]">
                                <p class="text-slate-450 font-semibold uppercase tracking-wider">Current Background Image</p>
                                <p class="text-slate-600 font-mono mt-0.5">{{ basename($slider->background_image) }}</p>
                            </div>
                        </div>
                    @endif
